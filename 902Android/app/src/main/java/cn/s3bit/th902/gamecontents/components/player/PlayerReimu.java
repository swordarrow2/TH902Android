...
public class PlayerReimu extends Player {
	...
	public void Initialize(Entity entity) {
		...
		mRenderer.attachToGroup(FightScreen.instance.groupNormal);
	...
	public void Update() {
		...
		if (transform.position.x - lastPos.x > 0 && animationDrawable.getAnimation() != animationRight) {
			animationDrawable.setAnimation(animationRight);
		} else if (transform.position.x - lastPos.x < 0 && animationDrawable.getAnimation() != animationLeft) {
			animationDrawable.setAnimation(animationLeft);
		} else if (transform.position.x == lastPos.x  && animationDrawable.getAnimation() != animationStay) {
			animationDrawable.setAnimation(animationStay);
		}
		wingShoot = bombFrames <= 60;
