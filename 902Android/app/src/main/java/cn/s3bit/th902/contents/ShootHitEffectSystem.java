...
public class ShootHitEffectSystem {
	...
	static {
		for (int i=0; i<pool.length; i++)
		{
			...
			effect.load(Gdx.files.external("resources/Particles/ShootHitEffect.dat"), Gdx.files.external("resources/Particles"));
			pool[i] = effect;
		}
	}
	...
	public static void addEffect(Vector2 position) {
		...
		managers[poolPt].AddComponent(new ParticleRenderer(pool[poolPt]).attachToGroup(FightScreen.instance.groupNormal));
		pool[poolPt++].start();
